...
def main():
 ...
 p.add_argument("--model-dir",default="outputs/xen-gen1-i")
 p.add_argument("--prompt",required=True)
 p.add_argument("--image",help="Reference image for image-to-image editing")
 p.add_argument("--strength",type=float,default=0.65,help="Edit strength, 0.05 preserves more of the input, 1.0 changes more")
 p.add_argument("--output",default="outputs/xen-gen1-i/generated.png")
 ...
 ids=torch.tensor([t.encode(a.prompt,max_length=256)],device=d); cond=c(ids)
 g=torch.Generator(device=d).manual_seed(a.seed)
 if a.image:
  base=load_image(a.image,a.size,d)
  start=max(1,min(999,round(999*a.strength)))
  ts=torch.linspace(start,0,a.steps,device=d)
  ...
  ts=torch.linspace(999,0,a.steps,device=d)
  x=torch.randn((1,3,a.size,a.size),device=d,generator=g)
 ...
 Path(a.output).parent.mkdir(parents=True,exist_ok=True); iio.imwrite(a.output,arr); print("Saved image:",a.output)
