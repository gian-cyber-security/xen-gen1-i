...
import json
class XENTokenizer:
    def __init__(self): self.vocab_size=260; self.special={"<pad>":0,"<bos>":1,"<eos>":2,"<unk>":3}
    ...
    @classmethod
    def load(cls,path):
        if json.loads(Path(path).read_text(encoding="utf-8")).get("type")!="byte": raise ValueError("Invalid XEN tokenizer")
        return cls()
